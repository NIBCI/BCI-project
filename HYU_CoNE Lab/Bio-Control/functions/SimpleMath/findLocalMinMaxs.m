...
% tol 은 미세하게 차이가 나는 것 때문에 min/max 가 아닌 것을 min/max라고 표시하는 현상을 막기 위함이다.
% supplimentary 폴더의 necessary_for_tolerance_2findLocalMinMax.mat 데이터를 사용하여
% 테스트해 보면 알 수 있다.
%----------------------------------------------------------------------
...
%---------------------------------------------------------------------
function [mins, maxs] = findLocalMinMaxs(data, tol)
    ...
    for i=2:nRow-1
        ...
        if(data(i)+tol <data(i-1) && data(i)+tol < data(i+1)) %min
            min_count = min_count+1;
            mins(min_count) = i;
            bTmpUp = 0;     bTmpDown = 0;
        elseif(data(i)-tol >data(i-1) && data(i)-tol  > data(i+1)) %max
