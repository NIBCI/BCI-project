...
num_classes = 6  # rest, right, left, front, up, down
class_order = [3, 0, 1, 2, 4, 5]
num_iteration = len(class_order)

folder = "Data"
data_type = "EEG_BallSix"
...
class BallRecorderSix:
    def __init__(self, subject="test", nChannels=31, frequency=512, streamer_type="OpenVibe", channels=None):
        ...
        self.model = FBCSP(augment=False, nChannels=total_ch, frequency=freq, chnames=channel_names)
    ...
    def draw_test(flag):
        ...
        if flag == 0:
            ...
            pygame.draw.rect(screen, (255, 0, 0), (0, 0, XSCREEN, circle_radius))
            pygame.draw.circle(screen, (0, 0, 255), (int(XSCREEN / 2), YSCREEN - circle_radius * 2), circle_radius)
        elif flag == 5:
            pygame.draw.rect(screen, (255, 255, 255), (0, 0, XSCREEN, circle_radius))
            pygame.draw.circle(screen, (0, 0, 255), (int(XSCREEN / 2), YSCREEN - circle_radius * 2), circle_radius)
        else:
            assert False
        pygame.display.update()
    ...
    def test_model(self):
        ...
        for t in range(num_tests * 3):
            ...
            if size_right <= size_left and size_right <= size_front:
                ...
                flag = 3
            '''
            class_determine=[100000000, size_right, size_left, size_front, size_up, size_down]
            ...
            while circle_radius < circle_x < XSCREEN - circle_radius and circle_y > circle_radius and 0 < circle_z < 2550:
                ...
                circle_x, circle_y, circle_z = self.update_circle(circle_x, circle_y, circle_z, v_x, v_y, v_z)
                time.sleep(0.01)
            ...
            prev_time = 0
            size_rest, size_right, size_left, size_front, size_up, size_down = self.class_count
            while size_rest < min(size_left, size_right, size_front, size_up, size_down):
                current_time = int(math.floor(time.clock() - start_time))
                if keyboard.is_pressed('m'):
                    time.sleep(2)
                    break
