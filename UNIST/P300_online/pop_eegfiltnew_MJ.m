% pop_eegfiltnew() - Filter sig.data using Hamming windowed sinc FIR filter
%
% Usage:
...
function [sig, com, b] = pop_eegfiltnew_MJ(sig, locutoff, hicutoff, filtorder, revfilt, usefft, plotfreqz, minphase)
...
if isempty(sig.data)
    error('Cannot filter empty sig.dataset.');
end
...
if nargin < 2
    ...
    uilist = {{'style', 'text', 'string', 'Lower edge of the frequency pass band (Hz)'} ...
              ...
              {'style', 'checkbox', 'string', 'Notch filter the sig.data instead of pass band', 'value', 0} ...
    ...
    result = inputgui('geometry', geometry, 'geomvert', geomvert, 'uilist', uilist, 'title', 'Filter the sig.data -- pop_eegfiltnew()', 'helpcom', 'pophelp(''pop_eegfiltnew'')');
...
if isempty(filtorder)
    ...
    filtorder = 3.3 / (df / sig.srate); % Hamming window
    filtorder = ceil(filtorder / 2) * 2; % Filter order must be even.
    
else

    df = 3.3 / filtorder * sig.srate; % Hamming window
    filtorderMin = ceil(3.3 ./ ((maxDf * 2) / sig.srate) / 2) * 2;
    filtorderOpt = ceil(3.3 ./ (maxDf / sig.srate) / 2) * 2;
...
if plotfreqz
    freqz(b, 1, 8192, sig.srate);
end

% Filter
if minphase
%     disp('pop_eegfiltnew() - filtering the sig.data (causal)');
    sig.data = firfiltsplit(sig.data, b, 1);
else
%     disp('pop_eegfiltnew() - filtering the sig.data (zero-phase)');
    sig.data = firfilt_MJ(sig.data, b);
end
